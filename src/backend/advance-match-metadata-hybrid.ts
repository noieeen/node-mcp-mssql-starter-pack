...
interface MetadataMatch {
    type: "table" | "column" | "template" | "relationship";
    id: string;
    name: string;
    description: string;
...
export interface QueryConfig {
    ...
    // You can add other config flags here if needed in the future
}

let businessDict: string[] = [];
...
// 1) Push your business terms up front (only once, not inside a ternary).
    if (hasThai === "th") {
        businessDict.push(
            "ลูกค้า",
            "ที่อยู่", "อาศัย",
            "ยอดขาย", "ขาย", "ชื้อ", "รายได้",
            "สินค้า",
            "สมาชิก",
            "โปรโมชั่น",
            "ทั้งหมด", "ร้าน", "บืล", "สาขา", "วันนี้", "วีคนี้", "เดือนนี้", "ราคา", "ผิดปกติ", "ยกเลิก"
        );
    }
...
// → "ข้อมูล|ลูกค้า|ทั้งหมด|ยอดขาย"
    ...
    const expanded = pattern.split("|").flatMap((w: any) => synonyms[w] ?? [w]);
    // Ensure expanded is string[] (type guard)
    const final = Array.from(new Set(expanded.filter((x): x is string => typeof x === 'string'))).slice(0, 10);
    console.log('final', final)

    return final
}

async function advanceMatchMetadataHybrid(config: QueryConfig): Promise<MetadataMatch[]> {
    try {
        ...
        const data = await getReferenceMetadataHybrid(
            question,
            searchPattern,
            lang,
            max_results,
