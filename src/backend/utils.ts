import {BCRM_TableSchema} from "../types.js";
...
function reduceMetadata(fullMetadata: {
    result: BCRM_TableSchema[];
}): ReducedColumn[] {
    if (!Array.isArray(fullMetadata.result)) return [];
...
function inferRole(col: BCRM_TableSchema): string {
    if (col.is_primary_key) return "primary_key";
    if (/id|ref/i.test(col.column_name)) return "foreign_key_or_id";
    if (/name|title/i.test(col.column_name)) return "text";
    if (/email|mobile/i.test(col.column_name)) return "contact";
    if (/spending|point|balance/i.test(col.column_name)) return "metric";
    if (col.data_type.includes("datetime")) return "datetime";
    if (/note/i.test(col.column_name)) return "irrelevant";
    if (/isdeleted|isdummy/i.test(col.column_name)) return "irrelevant";
    return "other";
}
...
const fullTypeTransform = (data_type: string, max_length?: number) => {
    if (!max_length) return data_type
...
const langDetect = (text: string): "en" | "th" | "mixed" => {
    const hasThai = /[\u0E00-\u0E7F]/.test(text);
    const hasEnglish = /[a-zA-Z]/.test(text);

    let lang: "en" | "th" | "mixed" = "en";
    if (hasThai && hasEnglish) lang = "mixed";
    else if (hasThai) lang = "th";
    return lang
}

const hasThai = (text: string): "en" | "th" => {
    const hasThai = /[\u0E00-\u0E7F]/.test(text);
 ...
 */
function manualCut(text: string, businessDict: string[]): string[] {
