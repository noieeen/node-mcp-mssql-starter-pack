...
export async function loadBusinessDict(lang?: string): Promise<string[]> {
    ...
    const cached = await getCache<string[]>(baseKey);
    console.log("cached", baseKey, cached);
    if (Array.isArray(cached)) return cached;

    const businessDict: string[] = [];
    ...
    if (tablesRes.data) {
        for (const t of tablesRes.data) {
            ...
            // @ts-ignore
            if (t[displayKey]) businessDict.push(t[displayKey].toLowerCase());
        }
    }
    ...
    if (colsRes.data) {
        for (const c of colsRes.data) {
            ...
            // @ts-ignore
            if (c[displayKey]) businessDict.push(c[displayKey].toLowerCase());
        }
    }
