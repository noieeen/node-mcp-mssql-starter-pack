...
// Define query template structure
...
export function registerQueryTemplateTools(server: McpServer) {
            ...
            try {
                ...
                const matches = QUERY_TEMPLATES.filter(template => {
                    ...
                    if (search_in === "all" || search_in === "tags") {
                        if (template.tags.some(tag => tag.toLowerCase().includes(searchLower))) return true;
                    }
                    return false;
                });
